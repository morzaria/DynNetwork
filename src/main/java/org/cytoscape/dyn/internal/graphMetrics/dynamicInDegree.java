...
public class dynamicInDegree<T> extends AbstractTask{
	
	private DynNetworkViewManagerImpl<T> dynNetViewManager;
	HashMap<DynInterval<T>,HashMap<CyNode,Integer>> nodeTimeDegreeMap;
	
	public dynamicInDegree(DynNetworkViewManagerImpl<T> dynNetViewManager){
	...
	public void run(TaskMonitor monitor){
		...
		nodeTimeDegreeMap=new HashMap<DynInterval<T>,HashMap<CyNode,Integer>>();
		...
		while(iterator.hasNext()){
			...
			nodeTimeDegreeMap.put((DynInterval<T>) snapshotInterval, new HashMap<CyNode, Integer>());
			while(nodeIterator.hasNext()){
				node=nodeIterator.next();
				((HashMap<CyNode, Integer>) nodeTimeDegreeMap.get(snapshotInterval)).put(node,networkSnapshot.inDegree(node));
